...
conflicts_prefer(flextable::width)
```
...
write.csv(eqtl_bygene_dirchanges_meta, "eqtl_bygene_dirchanges_meta.txt", row.names = FALSE)
```
...
write.csv(genereg_only_filt, "genereg_only_filt.txt", row.names = FALSE)
```

# Tables 1-3 - smaller versions
## Table 1: META genic SNPs associated with sepsis (simple)
```{r}
library(gt)
...
#Make the table
table1_simple_infection_ingene_meta <- infection_meta_snps_ingene %>% select(Gene, rsid, condition, alt, maf, or, pval, RDB, CADD) %>% group_by(condition) %>% 
  gt(rowname_col = "rsid") %>% 
  tab_spanner(
    label = "Annotation",
    columns = c(Gene,alt)
  ...
  tab_stubhead(
    label = md("**Phenotype and Variant**")
  ) %>% 
  cols_align(
    align = "center",
    columns = everything()
  ...
  tab_footnote(
    footnote = md("**Table 1: Genic AP SNPs associated with bacteremia phenotypes in MVP meta-analysis.** Phenotypes obtained from Phecode descriptions. All SNPs listed were obtained from MVP trans-ancestral meta-analyses (calculated via GWAMA) and met gene-specific significance thresholds obtained by Li and Ji transformation. RDB and CADD scores are included to reflect predicted effects on regulatory elements and SNP deleteriousness via *in silico* modeling. There was no eQTL data available for these SNPs so it is not included in this table. Exonic SNPs (synonymous and non-synonymous) are highlighted in bold. For significant SNPs in linkage disequilibrium (LD), only the \"lead\" SNP is listed here with secondary SNPs included as footnotes. Lead SNP selected based on p-value, functional impact, proximity to regulatory elements, and eQTL data.")
  ) %>% 
  tab_style(
    style = cell_text(weight = "bold"),
...
for (i in seq_len(nrow(infection_meta_snps_ingene))) {
  if (!is.na(infection_meta_snps_ingene$secondary_rsid_combined[i]) &&
    ...
    table1_simple_infection_ingene_meta <- table1_simple_infection_ingene_meta %>%
      tab_footnote(
...
table1_simple_infection_ingene_meta

gtsave(table1_simple_infection_ingene_meta, "table1_simple_infection_ingene_meta.docx")

gtsave(table1_simple_infection_ingene_meta, "table1_simple_infection_ingene_meta.html")
webshot(url = "table1_simple_infection_ingene_meta.html", file = "table1_simple_infection_ingene_meta.pdf")
```


## Table 2: META genic SNPs associated with sepsis (simple)
...
inflammation_meta_snps_ingene <- inflammation_meta_snps_ingene %>% rowwise %>% 
  separate_longer_delim(eqtl_genes_complement, delim = ",") %>% 
  mutate(
    eqtl_genes_direction = ifelse(
      !is.na(eqtl_genes_complement) & eqtl_genes_complement != "",
      paste0(eqtl_genes_complement, " (", eqtl_direction, ")"),
      NA
    )) %>% 
  group_by(rsid) %>% 
  summarize(eqtl_genes_direction_combined = paste(unique(na.omit(eqtl_genes_direction)), collapse = ", "), 
            across(everything(), ~first(., na.rm = TRUE))) %>% 
  ungroup() %>% 
  rowwise() %>% 
  mutate(
    eqtl_present = ifelse(!is.na(eqtl_genes_direction_combined) & eqtl_genes_direction_combined != "", 1, 0)
    ) %>% 
  ungroup()

#Make the table
table2_simple_inflammation_ingene_meta <- inflammation_meta_snps_ingene %>%  select(Gene, rsid, condition, alt, maf, or, pval, RDB, CADD, eqtl_genes_direction_combined, eqtl_present) %>% group_by(condition) %>% 
  gt(rowname_col = "rsid") %>% 
  tab_spanner(
    label = "Annotation",
    columns = c(Gene,alt)
  ...
  tab_footnote(
    footnote = md("**Table 2: Genic AP SNPs associated with sepsis phenotypes in MVP meta-analysis.** Phenotypes obtained from Phecode descriptions. All SNPs listed were obtained from MVP trans-ancestral meta-analyses (calculated via GWAMA) and met gene-specific significance thresholds obtained by Li and Ji transformation. RDB and CADD scores are included to reflect predicted effects on regulatory elements and SNP deleteriousness via *in silico* modeling. The eQTL data for SNPs marked with an asterisk can be viewed in the supplement (Table S1). Exonic SNPs (synonymous and non-synonymous) are highlighted in bold. For significant SNPs in linkage disequilibrium (LD), only the \"lead\" SNP is listed here with secondary SNPs included as footnotes. Lead SNP selected based on p-value, functional impact, proximity to regulatory elements, and eQTL data.")
  ) %>% 
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(columns = everything(), rows = rsid %in% coding_snps)
  ...
  text_transform(
    locations = cells_stub(
      rows = which(eqtl_present==1, arr.ind = TRUE)
      ),
    fn = function(x) {
      paste0(x, "<sup>*</sup>")
      }
  ...
  cols_hide(
    columns = c(eqtl_genes_direction_combined, eqtl_present)
  )
...
for (i in seq_len(nrow(inflammation_meta_snps_ingene))) {
  if (!is.na(inflammation_meta_snps_ingene$secondary_rsid_combined[i]) &&
    ...
    table2_simple_inflammation_ingene_meta <- table2_simple_inflammation_ingene_meta %>%
      tab_footnote(
...
if(any(!is.na(inflammation_meta_snps_ingene$eqtl_genes_direction_combined))) {
  table2_simple_inflammation_ingene_meta <- table2_simple_inflammation_ingene_meta %>%
    tab_footnote(
      footnote = md("<sup>*</sup>Known association with alternative pathway gene expression based on eQTL data, see supplement for details.")
    )
}

table2_simple_inflammation_ingene_meta

gtsave(table2_simple_inflammation_ingene_meta, "table2_simple_inflammation_ingene_meta.docx")

gtsave(table2_simple_inflammation_ingene_meta, "table2_simple_inflammation_ingene_meta.html")
webshot(url = "table2_simple_inflammation_ingene_meta.html", file = "table2_simple_inflammation_ingene_meta.pdf")
```

## Table 3: META potential regulatory SNPs associated with bacteremia AND sepsis (simple)
...
#Make the table
table3_simple_combined_meta_regonly <- combined_snps_meta_regonly %>% select(Gene, rsid, condition, alt, maf, or, pval, RDB, CADD) %>% group_by(condition) %>% 
  gt(rowname_col = "rsid") %>% 
  tab_spanner(
    label = "Annotation",
    columns = c(Gene, alt)
  ...
  tab_footnote(
    footnote = md("**Table 3: Potential regulatory AP SNPs associated with bacteremia or sepsis phenotypes in MVP meta-analysis.** Phenotypes obtained from Phecode descriptions. For significant SNPs in linkage disequilibrium (LD), only the \"lead\" SNP is listed here with secondary SNPs included as footnotes. Lead SNP selected based on p-value, functional impact, proximity to regulatory elements, and eQTL data. All listed SNPs met gene-specific adjusted significance thresholds obtained by Li and Ji transformation in MVP trans-ancestral meta-analysis (obtained with GWAMA) or are in strong LD with a SNP that is significant in MVP trans-ancestral meta-analysis. Odds ratios are based on presence or absence of minor allele, which is treated as the risk allele. P-value, odds ratios, and MAF for SNPs that are significant in multiple groups are based on MVP meta-analysis when available or the largest ancestral group based on number of cases reported when meta-analysis data are unavailable. Gene association was determined by FUMA and ANNOVAR annotation or by localization within an AP gene enhancer region as defined by EnhancerAtlas. Enhancer localization is available in the full version of this table (Table S2). Only SNPs with a high predicted functional impact by *in silico* modeling (CADD >12.37 or RDB 3b or lower) are included here.")
  )
...
for (i in seq_len(nrow(combined_snps_meta_regonly))) {
  if (!is.na(combined_snps_meta_regonly$secondary_rsid_combined[i]) &&
    ...
    table3_simple_combined_meta_regonly <- table3_simple_combined_meta_regonly %>%
      tab_footnote(
...
#Print the table
#table3_simple_combined_meta_regonly

gtsave(table3_simple_combined_meta_regonly, "table3_simple_combined_meta_regonly.docx")

#Save for later publication
gtsave(table3_simple_combined_meta_regonly, "table3_simple_combined_meta_regonly.html")
webshot(url = "table3_simple_combined_meta_regonly.html", file = "table3_simple_combined_meta_regonly.pdf", vwidth = 1200, vheight = 800)
```
...
gtsave(table4_combined_eqtlonly_bygene_meta, "table4_combined_eqtlonly_bygene_meta.docx")
...
# Export as Word documents with formatting preserved
## Table 1
### Prepare the data
```{r}
...
# Create a flextable
table1_ft_data <- infection_meta_snps_ingene %>% 
  select(condition, rsid, Gene, alt, maf, or, pval, RDB, CADD, secondary_rsid_combined, secondary_meta) %>% 
  group_by(condition) %>% 
  arrange(rsid, .by_group = TRUE) %>%
  group_split() %>%
  map_dfr(function(df_group) {
    cond <- unique(df_group$condition)
    ...
    header_row <- tibble(
      ...
      RDB = NA,
      CADD = NA,
      secondary_rsid_combined = NA,
      secondary_meta = NA
    )
    
    # Indent rsid values
    df_group <- df_group %>%
      mutate(rsid = paste0("     ", rsid))
    
    bind_rows(header_row, df_group)
  })

# Identify rows that need an LD footnote
ld_rows <- which(!is.na(table1_ft_data$secondary_rsid_combined) &
                 table1_ft_data$secondary_rsid_combined != "")

# Extract the actual notes
ld_notes_raw <- table1_ft_data$secondary_rsid_combined[ld_rows]

# Identify unique notes → map them to letters/numbers
unique_notes <- unique(ld_notes_raw)
note_ids <- match(ld_notes_raw, unique_notes)  # assign index of 1st occurrence

# Build full formatted footnotes (deduplicated)
ld_notes <- paste0("Additional SNPs in LD: ", unique_notes)

table1_ft_data <- table1_ft_data %>%
  mutate(across(c("maf", "or"), ~ signif(., 3))) %>% 
  mutate(pval = ifelse(!is.na(pval), formatC(pval, format = "e", digits = 2), NA))
```

### Create the flextable and export to Word
```{r}
table1_ft <- table1_ft_data %>% select(rsid, Gene, alt, maf, or, pval, RDB, CADD) %>%
  flextable() %>% 
  bold(part = "header") %>% 
  bold(i = grepl(paste(coding_snps, collapse = "|"), table1_ft_data$rsid), part = "body") %>% 
  set_caption(
    caption = as_paragraph(
      as_chunk("Table 1. Genic AP SNPs Associated with Bacteremia Phenotypes in MVP Meta-Analysis",
               props = fp_text(font.size = 12, font.family = "Arial", bold = TRUE))
      )) %>% 
  add_header_row(
    values = c("", "Annotation", "MVP Meta-Analysis Data", "Predictive Modeling"),
    colwidths = c(1, 2, 3, 2)
  ) %>% 
  bold(i = 1, bold = FALSE, part = "header") %>% 
  set_header_labels(
    rsid = "Phenotype and Variant",
    alt = "Minor Allele",
    or = "OR",
    maf = "MAF",
    pval = "P-Value"
  ...
  add_footer_lines(
    value = as_paragraph(
      as_chunk("Table 1: Genic AP SNPs associated with bacteremia phenotypes in MVP meta-analysis.", 
               props = fp_text(font.size = 9, bold = TRUE, font.family = "Arial")), 
      as_chunk(" Table 1: Genic AP SNPs associated with bacteremia phenotypes in MVP meta-analysis.** Phenotypes obtained from Phecode descriptions. All SNPs listed were obtained from MVP trans-ancestral meta-analyses (calculated via GWAMA) and met gene-specific significance thresholds obtained by Li and Ji transformation. RDB and CADD scores are included to reflect predicted effects on regulatory elements and SNP deleteriousness via ", props = fp_text(font.size = 9, bold = FALSE, font.family = "Arial")), 
               as_chunk("in silico", props = fp_text(font.size = 9, italic = TRUE, font.family = "Arial")), 
               as_chunk(" modeling. There was no eQTL data available for any of the SNPs in this table. Exonic SNPs (synonymous and non-synonymous) are highlighted in bold. For significant SNPs in linkage disequilibrium (LD), only the \"lead\" SNP is listed here with secondary SNPs included as footnotes. Lead SNP selected based on p-value, functional impact, proximity to regulatory elements, and eQTL data.", props = fp_text(font.size = 9, bold = FALSE, font.family = "Arial"))
    )
  ) %>% 
  ...
  width(
    j = c("rsid", "Gene", "alt", "maf", "or", "pval", "RDB", "CADD"),
    width = unit(c(1.5, 1, 1, 1, 1, 1, 1, 1), "in")
  ) %>% 
  set_table_properties(layout = "fixed", width = 0.95)
  
# Apply footnotes in one call
table1_ft <- footnote(
  table1_ft,
  i          = ld_rows,     # rows in body
  j          = 1,           # stub/first column
  value      = as_paragraph(ld_notes[note_ids]),
  ref_symbols = note_ids,  # a, b, c, …
  part       = "body",
  inline     = FALSE        # one footer row per note
)

# Export flextable to landscape Word document
section_landscape <- prop_section(
...
# Create a flextable
table2_ft_data <- inflammation_meta_snps_ingene %>% 
  select(condition, rsid, Gene, alt, maf, or, pval, RDB, CADD, secondary_rsid_combined, secondary_meta, eqtl_present) %>% 
  group_by(condition) %>% 
  arrange(rsid, .by_group = TRUE) %>%
  group_split() %>%
  map_dfr(function(df_group) {
...
# Identify rows that need an LD footnote
ld_rows2 <- which(!is.na(table2_ft_data$secondary_rsid_combined) &
                 table2_ft_data$secondary_rsid_combined != "")

# Extract the actual notes
ld_notes_raw2 <- table2_ft_data$secondary_rsid_combined[ld_rows2]

# Identify unique notes → map them to letters/numbers
unique_notes2 <- unique(ld_notes_raw2)
note_ids2 <- match(ld_notes_raw2, unique_notes2)  # assign index of 1st occurrence

# Build full formatted footnotes (deduplicated)
ld_notes2 <- paste0("Additional SNPs in LD: ", unique_notes2)

eqtl_present_mark <- which(table2_ft_data$eqtl_present==1)

table2_ft_data <- table2_ft_data %>%
  mutate(across(c("maf", "or"), ~ signif(., 3))) %>% 
  mutate(pval = ifelse(!is.na(pval), formatC(pval, format = "e", digits = 2), NA))
```
...
### Create the flextable and export to Word
```{r}
table2_ft <- table2_ft_data %>% select(rsid, Gene, alt, maf, or, pval, RDB, CADD, eqtl_present) %>%
  flextable() %>% 
  bold(part = "header") %>% 
  bold(i = grepl(paste(coding_snps, collapse = "|"), table2_ft_data$rsid), part = "body") %>% 
  set_caption(
    caption = as_paragraph(
      as_chunk("Table 2. Genic AP SNPs Associated with Sepsis Phenotypes in MVP Meta-Analysis",
               props = fp_text(font.size = 12, font.family = "Arial", bold = TRUE))
      )) %>% 
  add_header_row(
    values = c("", "Annotation", "MVP Meta-Analysis Data", "Predictive Modeling"),
  ...
  add_footer_lines(
    value = as_paragraph(
      as_chunk("Table 2: Genic AP SNPs associated with sepsis phenotypes in MVP meta-analysis.", 
               props = fp_text(font.size = 8, bold = TRUE, font.family = "Arial")), 
      as_chunk(" All SNPs listed were obtained from MVP trans-ancestral meta-analyses (calculated via GWAMA) and met gene-specific significance thresholds obtained by Li and Ji transformation. RDB and CADD scores are included to reflect predicted effects on regulatory elements and SNP deleteriousness via ", props = fp_text(font.size = 8, bold = FALSE, font.family = "Arial")),
      as_chunk("in silico", props = fp_text(font.size = 8, italic = TRUE, font.family = "Arial")),
      as_chunk (" modeling. The eQTL data for SNPs marked with an asterisk can be viewed in the supplement (Table S1). Exonic SNPs (synonymous and non-synonymous) are highlighted in bold. For significant SNPs in linkage disequilibrium (LD), only the \"lead\" SNP is listed here with secondary SNPs included as footnotes. Lead SNP selected based on p-value, functional impact, proximity to regulatory elements, and eQTL data.", props = fp_text(font.size = 8, bold = FALSE, font.family = "Arial"))
    )
  ) %>% 
  fontsize(size = 10, part = "header") %>% 
  ...
  align(part = "body", align = "center") %>% 
  align(part = "body", j = 1, align = "left") %>% 
  padding(padding = 2.5, part = "header") %>% 
  padding(padding = 2, part = "body") %>% 
  padding(padding = 1.5, part = "footer") %>%
  width(
    j = c("rsid", "Gene", "alt", "maf", "or", "pval", "RDB", "CADD", "eqtl_present"),
    width = unit(c(1.5, 1, 1, 1, 1, 1, 1, 1, 0), "in")
  ) %>% 
  set_table_properties(layout = "fixed", width = 0.95)
  
# Apply footnotes in one call
for (k in seq_along(unique_notes2)) {
  rows_k <- ld_rows2[note_ids2 == k]
  table2_ft <- footnote(
    table2_ft,
    i = rows_k,
    j = 1,
    value = as_paragraph(ld_notes2[k]),
    ref_symbols = k,
    part = "body",
    inline = FALSE
  )
...
if (any(table2_ft_data$eqtl_present==1)) {
  table2_ft <- footnote(
    table2_ft,
    i = eqtl_present_mark,
    j = 1,
    ...
    value = as_paragraph(
      as_sup("*"), 
      "Known association with alternative pathway gene expression based on eQTL data, see supplement for details."
    )
  )
}
...
# Create a flextable
table3_ft_data <- combined_snps_meta_regonly %>% 
  select(Gene, rsid, condition, alt, maf, or, pval, RDB, CADD, secondary_rsid_combined, secondary_meta) %>% 
  mutate(infection_condition = ifelse(condition %in% infection_conditions, 1, 0))

# Create a factor for condition with infection_condition groups first
cond_levels <- table3_ft_data %>%
  distinct(condition, infection_condition) %>%
  arrange(desc(infection_condition), condition) %>%
  pull(condition)

# Then convert to factor with levels ordered accordingly
table3_ft_data <- table3_ft_data %>%
  mutate(condition = factor(condition, levels = cond_levels)) %>%
  arrange(condition, rsid) %>% 
  group_by(condition) %>% 
  arrange(rsid, .by_group = TRUE) %>%
  group_split() %>%
  map_dfr(function(df_group) {
...
# Identify rows that need an LD footnote
ld_rows3 <- which(!is.na(table3_ft_data$secondary_rsid_combined) &
                 table3_ft_data$secondary_rsid_combined != "")

# Extract the actual notes
ld_notes_raw3 <- table3_ft_data$secondary_rsid_combined[ld_rows3]

# Identify unique notes → map them to letters/numbers
unique_notes3 <- unique(ld_notes_raw3)
note_ids3 <- match(ld_notes_raw3, unique_notes3)  # assign index of 1st occurrence

# Build full formatted footnotes (deduplicated)
ld_notes3 <- paste0("Additional SNPs in LD: ", unique_notes3)
...
### Create the flextable and export to Word
```{r}
table3_ft <- table3_ft_data %>% select(rsid, Gene, alt, maf, or, pval, RDB, CADD) %>%
  flextable() %>% 
  bold(part = "header") %>% 
  set_caption(
    caption = as_paragraph(
      as_chunk("Table 3. Potential Regulatory AP SNPs Associated with Bacteremia or Sepsis Phenotypes in MVP Meta-Analysis", props = fp_text(font.size = 12, font.family = "Arial", bold = TRUE))
      )) %>% 
  add_header_row(
    values = c("", "Annotation", "MVP Meta-Analysis Data", "Predictive Modeling"),
    colwidths = c(1, 2, 3, 2)
  ...
  add_footer_lines(
    value = as_paragraph(
      as_chunk("Table 3: Potential regulatory AP SNPs associated with bacteremia or sepsis phenotypes in MVP meta-analysis.", props = fp_text(font.size = 10, bold = TRUE, font.family = "Arial")),
      as_chunk(" Phenotypes obtained from Phecode descriptions. For significant SNPs in linkage disequilibrium (LD), only the \"lead\" SNP is listed here with secondary SNPs included as footnotes. Lead SNP selected based on p-value, functional impact, proximity to regulatory elements, and eQTL data. All listed SNPs met gene-specific adjusted significance thresholds obtained by Li and Ji transformation in MVP trans-ancestral meta-analysis (obtained with GWAMA) or are in strong LD with a SNP that is significant in MVP trans-ancestral meta-analysis. Odds ratios are based on presence or absence of minor allele, which is treated as the risk allele. P-value, odds ratios, and MAF for SNPs that are significant in multiple groups are based on MVP meta-analysis when available or the largest ancestral group based on number of cases reported when meta-analysis data are unavailable. Gene association was determined by FUMA and ANNOVAR annotation or by localization within an AP gene enhancer region as defined by EnhancerAtlas. Enhancer localization is available in the full version of this table (Table S2). Only SNPs with a high predicted functional impact by ", props = fp_text(font.size = 10, bold = FALSE, font.family = "Arial")),
      as_chunk("in silico", props = fp_text(font.size = 10, italic = TRUE, font.family = "Arial")),
      as_chunk(" modeling (CADD >12.37 or RDB 3b or lower) are included.", props = fp_text(font.size = 10, font.family = "Arial"))
    )
  ) %>% 
  fontsize(size = 10, part = "header") %>% 
  ...
  align(part = "body", align = "center") %>% 
  align(part = "body", j = 1, align = "left") %>% 
  padding(padding = 3, part = "header") %>% 
  padding(padding = 2, part = "body") %>% 
  padding(padding = 2, part = "footer") %>%
  width(
    j = c("rsid", "Gene", "alt", "maf", "or", "pval", "RDB", "CADD"),
    width = unit(c(1.25, 1, 1, 1, 1, 1, 1, 1), "in")
  ) %>% 
  set_table_properties(layout = "fixed", width = 0.95)
  
# Apply footnotes
for (k in seq_along(unique_notes3)) {
  rows_k <- ld_rows3[note_ids3 == k]
  table3_ft <- footnote(
    table3_ft,
    i = rows_k,
    j = 1,
    value = as_paragraph(ld_notes3[k]),
    ref_symbols = k,
    part = "body",
    inline = FALSE
  )
...
# Create a flextable
table4_ft_data <- combined_snps_eqtlonly_bygene_meta %>% 
  select(condition, eqtl_genes_complement, eqtl_direction_complement, eqtl_tissues_complement, associated_rsids) %>% 
  mutate(infection_condition = ifelse(condition %in% infection_conditions, 1, 0))

# Create a factor for condition with infection_condition groups first
cond_levels <- table4_ft_data %>%
  distinct(condition, infection_condition) %>%
  arrange(desc(infection_condition), condition) %>%
  pull(condition)

# Then convert to factor with levels ordered accordingly
table4_ft_data <- table4_ft_data %>%
  mutate(condition = factor(condition, levels = cond_levels)) %>%
  arrange(condition, eqtl_genes_complement) %>% 
  group_by(condition) %>% 
  arrange(eqtl_genes_complement, .by_group = TRUE) %>%
  group_split() %>%
  map_dfr(function(df_group) {
    cond <- unique(df_group$condition)
    ...
    # Create a header row
    header_row <- tibble(
      eqtl_genes_complement = cond,
      eqtl_direction_complement = NA,
      eqtl_tissues_complement = NA,
      associated_rsids = NA
    )
    
    # Indent genes
    df_group <- df_group %>% 
      mutate(eqtl_genes_complement = paste0("           ", eqtl_genes_complement))
...
### Create the flextable and export to Word
```{r}
table4_ft <- table4_ft_data %>% select(eqtl_genes_complement, eqtl_direction_complement, eqtl_tissues_complement, associated_rsids) %>%
  flextable() %>% 
  bold(part = "header") %>% 
  set_caption(
    caption = as_paragraph(
      as_chunk("Table 4. Additional SNPs with AP eQTL Data That are Associated with Sepsis or Bacteremia Phenotypes in MVP Meta-Analysis", props = fp_text(font.size = 12, font.family = "Arial", bold = TRUE))
      )) %>% 
  set_header_labels(
  ...
  add_footer_lines(
    value = as_paragraph(
      as_chunk("Table 4: Additional SNPs with AP eQTL data that are associated with sepsis or bacteremia phenotypes in MVP meta-analysis.", props = fp_text(font.size = 8, bold = TRUE, font.family = "Arial")),
      as_chunk(" Phenotypes obtained from Phecode descriptions. eQTL data was obtained from GTEX and LDLink - the latter includes results from SNPs in strong LD with those listed here. All listed SNPs met adjusted significance thresholds obtained by Li and Ji transformation in trans-ancestral meta-analysis (obtained with GWAMA). Only SNPs beyond gene loci without significant ", props = fp_text(font.size = 8, bold = FALSE, font.family = "Arial")),
      as_chunk("in silico", props = fp_text(font.size = 8, italic = TRUE, font.family = "Arial")),
      as_chunk(" findings are included here. All listed SNPs are \"lead\" SNPs identified following LD clustering; secondary SNPs are not included in this list and are available in supplemental data. For eQTL data associated with genic SNPs, see Table 1. For SNPs with strong ", props = fp_text(font.size = 8, italic = TRUE, font.family = "Arial")),
      as_chunk("in silico", props = fp_text(font.size = 8, italic = TRUE, font.family = "Arial")),
      as_chunk(" predicted effects and eQTL data, see Table 2. Listed tissue types are based on the GTEX v8 database. Directionality was selected by association with an increased risk for the listed conditions.", props = fp_text(font.size = 8, font.family = "Arial"))
    )
  ) %>% 
  ...
  padding(padding = 2, part = "footer") %>%
  width(
    j = c("eqtl_genes_complement", "eqtl_direction_complement", "eqtl_tissues_complement", "associated_rsids"),
    width = unit(c(2, 2, 2, 4), "in")
  ) %>% 
  set_table_properties(layout = "fixed", width = 0.95)
...
tables5_textformat <- wide_snps_withld_filtered %>%
...
tables1_ljresults <- lj_results %>% select(Gene, Chromosome, Meff, sig_threshold_bon) %>% filter(!(Gene == "CFP")) %>% 
  gt(rowname_col = "Gene") %>% 
  cols_label(
  ...
  tab_header(
    title = md("**Table S1. Adjusted Significance Thresholds by Gene**")
  ) %>%
  tab_stubhead(
    label = md("**Gene**")
  ) %>% 
...
ld_summary_table <- ld_summary_collapsed %>% 
  gt() %>% 
  tab_header(
    title = md("**Table S2. LD Information for Genic and Regulatory SNPs**")
  )
gtsave(ld_summary_table, "ld_summary_table.html")
webshot(url = "ld_summary_table.html", file = "ld_summary_table.pdf", vwidth = 1200, vheight = 800)

eqtl_summary_table <- eqtl_ld_summary_collapsed %>% 
  gt() %>% 
  tab_header(
    title = md("**Table S3. LD Information for eQTL SNPs**")
  )
...
# Table S5: all genic SNPs across all races (Table S4 is RegulomeDB scoring)
```{r MVP-ExtendedData-Final-v1.7-64}
wide_genic_snps_withld_filtered <- wide_snps_withld_filtered %>% filter(in_gene == 1)

library(gt)
tables5_genic_snps_all <- wide_genic_snps_withld_filtered %>%  dplyr::select(Gene,alt,rsid,condition,AFR_or,AFR_pval,EUR_or,EUR_pval,HIS_or,HIS_pval,META_or,META_pval) %>% group_by(condition) %>% 
  gt(rowname_col = "rsid") %>% 
  tab_spanner(
    label = "Annotation",
    columns=c(Gene,alt)
  ...
  tab_header(
    title = md("**Table S5. Genic Alternative Pathway (AP) SNPs Associated with Sepsis and Bacteremia Phecodes Across All Races**")
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
  ...
  tab_footnote(
    footnote = md("**Table S5: Genic alternative pathway (AP) SNPs associated with sepsis and bacteremia phecodes across all races.** Phenotypes obtained from Phecode descriptions. All listed SNPs met gene-specific significance thresholds obtained by Li and Ji transformation in either individual ancestral groups  or trans-ancestral meta-analysis (obtained with GWAMA). P-values that met adjusted signifiance thresholds are highlighted in bold. Odds ratios are based on presence or absence of minor allele, which is treated as the risk allele. Gene assignment was determined by FUMA and ANNOVAR annotation.")
  )
...
for (i in seq_len(nrow(wide_genic_snps_withld_filtered))) {
  if (!is.na(wide_genic_snps_withld_filtered$secondary_rsid_combined[i]) &&
    ...
    tables5_genic_snps_all <- tables5_genic_snps_all %>%
      tab_footnote(
...
#tables5_genic_snps_all

gtsave(tables5_genic_snps_all, "tables5_genic_snps_all.html")
webshot(url = "tables5_genic_snps_all.html", file = "tables5_genic_snps_all.pdf", vwidth = 1200, vheight = 800)

gtsave(tables5_genic_snps_all, "tables5_genic_snps_all.docx")

write.table(tables5_textformat, "/Users/kinman/Desktop/MVP_Data/Supplementary\ Data/tables5_genic_snps_all.tsv", sep="\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
```

# Table S6: all regulatory SNPs across all races
...
#Make the table
tables6_regonly_all <- regonly_snps_combined %>% select(races_combined, Gene, rsid, condition, alt, maf, or, pval, RDB, CADD, enh_location, enh_tissue_all, annot) %>% group_by(condition) %>% 
  gt(rowname_col = "rsid") %>% 
  tab_spanner(
    label = "Annotation",
    columns = c(Gene, alt)
  ...
  tab_header(
    title = md("**Table S6. Potential Regulatory Alternative Pathway (AP) SNPs by Predictive Modeling Associated with Sepsis or Bacteremia Phenotypes Across All Races**")
  ) %>%
  tab_stubhead(
    label = md("**Phenotype**")
  ) %>% 
...
for (i in seq_len(nrow(regonly_snps_combined))) {
  if (!is.na(regonly_snps_combined$secondary_rsid_combined[i]) &&
    ...
    tables8_regonly_all <- tables8_regonly_all %>%
      tab_footnote(
...
if(any(regonly_snps_combined$annot == "exonic")) {
  tables6_regonly_all <- tables6_regonly_all %>%
    tab_footnote(
...
#Save for later publication
gtsave(tables6_regonly_all, "tables6_regonly_all.html")
webshot(url = "tables6_regonly_all.html", file = "tables6_regonly_all.pdf", vwidth = 1200, vheight = 800)

gtsave(tables6_regonly_all, "tables6_regonly_all.docx")
...
# Table S7: all extended eQTL data across all races
```{r Table S7 - eQTL for all races}
library(gt)
...
#Make the table
tables7_combined_eqtlonly_bygene <- combined_snps_eqtlonly_bygene %>% select(condition, eqtl_genes_complement, eqtl_direction_complement, eqtl_tissues_complement, associated_rsids) %>% group_by(condition) %>% 
  gt(rowname_col = "eqtl_genes_complement") %>% 
  cols_label(
    associated_rsids = "Associated rsIDs",
    eqtl_tissues_complement = "GTEX Tissues",
  ...
  tab_header(
    title = md("**Table S7. Additional SNPs with AP eQTL Data That are Associated with Sepsis or Bacteremia Phenotypes Across All Races**")
  ) %>%
  tab_stubhead(
    label = md("**Phenotype and Affected Gene**")
  ) %>% 
...
# Table S8: META genic SNPs associated with sepsis
```{r MVP-ExtendedData-Final-v1.7-60}
library(gt)
...
dups_tables8 <- inflammation_meta_snps_ingene %>% filter(duplicated(rsid)) %>% pull(rsid)
dups_tables8_data <- inflammation_meta_snps_ingene %>% filter(rsid %in% dups_tables8)
...
#Make the table
tables8_inflammation_ingene_meta <- inflammation_meta_snps_ingene %>%  select(races_combined, Gene, rsid, condition, alt, maf, or, pval, RDB, CADD, eqtl_genes_complement, eqtl_direction, eqtl_tissues_complement) %>% group_by(condition) %>% 
  gt(rowname_col = "rsid") %>% 
  tab_spanner(
    label = "Annotation",
    columns = c(Gene,alt)
  ...
  tab_header(
    title = md("**Table S8. Genic AP SNPs Associated with Sepsis Phenotypes in MVP Meta-Analysis**")
  ) %>%
  tab_stubhead(
    label = md("**Phenotype**")
  ) %>% 
...
for (i in seq_len(nrow(inflammation_meta_snps_ingene))) {
  if (!is.na(inflammation_meta_snps_ingene$secondary_rsid_combined[i]) &&
    ...
    tables8_inflammation_ingene_meta <- tables8_inflammation_ingene_meta %>%
      tab_footnote(
        footnote = paste0("Additional SNPs in LD: ", inflammation_meta_snps_ingene$secondary_rsid_combined[i]),
        locations = cells_stub(
          rows = i
...
#tables8_inflammation_ingene_meta

gtsave(tables8_inflammation_ingene_meta, "tables8_inflammation_ingene_meta.docx")

gtsave(tables8_inflammation_ingene_meta, "tables8_inflammation_ingene_meta.html")
webshot(url = "tables8_inflammation_ingene_meta.html", file = "tables8_inflammation_ingene_meta.pdf")
```

# Table S9: META potential regulatory SNPs associated with bacteremia AND sepsis (COMBINED TABLE)
```{r MVP-ExtendedData-Final-v1.7-61}
library(gt)
...
n_distinct(combined_snps_meta_regonly$rsid)
dups_table9 <- combined_snps_meta_regonly %>% filter(duplicated(rsid)) %>% pull(rsid)
dups_table9_data <- combined_snps_meta_regonly %>% filter(rsid %in% dups_table9)

#Make the table
tables9_combined_meta_regonly <- combined_snps_meta_regonly %>% select(races_combined, Gene, rsid, condition, alt, maf, or, pval, RDB, CADD, enh_location, enh_tissue_all) %>% group_by(condition) %>% 
  gt(rowname_col = "rsid") %>% 
  tab_spanner(
    label = "Annotation",
    columns = c(Gene, alt)
  ...
  tab_header(
    title = md("**Table S9. Potential Regulatory AP SNPs Associated with Bacteremia or Sepsis Phenotypes in MVP Meta-Analysis**")
  ) %>%
  tab_stubhead(
    label = md("**Phenotype**")
  ) %>% 
...
for (i in seq_len(nrow(combined_snps_meta_regonly))) {
  if (!is.na(combined_snps_meta_regonly$secondary_rsid_combined[i]) &&
    ...
    tables9_combined_meta_regonly <- tables9_combined_meta_regonly %>%
      tab_footnote(
        footnote = paste0("Additional SNPs in LD: ", combined_snps_meta_regonly$secondary_rsid_combined[i]),
        locations = cells_stub(
          rows = i
...
#Print the table
tables9_combined_meta_regonly

gtsave(tables9_combined_meta_regonly, "tables9_combined_meta_regonly.docx")

#Save for later publication
gtsave(tables9_combined_meta_regonly, "tables9_combined_meta_regonly.html")
webshot(url = "tables9_combined_meta_regonly.html", file = "tables9_combined_meta_regonly.pdf", vwidth = 1200, vheight = 800)
```
